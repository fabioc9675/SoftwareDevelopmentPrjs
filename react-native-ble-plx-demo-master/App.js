      ...
      device
        ...
        .then((characteristic) => {
          ...
          device.services().then((services) => {
            ...
            snifferService = services.filter(
              (service) => service.uuid === this.state.serviceUUID,
            )[0];
            snifferService
              .characteristics()
              .then((characteristics) => {
                ...
                voltageCharacteristic = characteristics.filter(
                  (c) => c.uuid === characteristics[0].uuid,
                )[0];
                voltageCharacteristic.monitor((error, c) => {
                  ...
                  if (error) {
                    ...
                    if (isNaN(s1)) {
                      ...
                      if (count == 1) {
                        this.state.makedata.push(
                          <Text key={moment().valueOf()}>
                            {s[0]} : {s1 / 1000} {'\n'}{' '}
                          </Text>,
                        );
                        this.setState({
